...
export async function fetchPlaceDetails(placeName: string): Promise<PlaceData> {
  ...
  try {
    ...
    console.log(
      `[Server][Firebase Debug] 🕵️ fetchPlaceDetails 조회 | 이름: "${placeName}"`,
    );
...
export async function getNearbyIndoorPlaces(
  lat: number,
  lng: number,
): Promise<PlanItem[]> {
  try {
...
function checkIsIndoor(place: FirebasePlace): boolean {
  ...
  if (
    place.CATEGORY.sub &&
    indoorKeywords.some((k) => place.CATEGORY.sub.includes(k))
  )
    return true;
  if (place.CATEGORY.main === "카페" || place.CATEGORY.main === "식당")
    return true;

  // 2. 키워드 체크
  if (
    place.KEYWORDS &&
    place.KEYWORDS.some((k) => indoorKeywords.some((ik) => k.includes(ik)))
  )
    return true;
  ...
  if (
    place.CATEGORY.sub &&
    outdoorKeywords.some((k) => place.CATEGORY.sub.includes(k))
  )
    return false;
...
export async function getPlacesByTheme(theme: string): Promise<PlanItem[]> {
  try {
    ...
    const snapshot = await getDocs(q);
    console.log(
      `[Server][Firebase Debug] 🎨 getPlacesByTheme 조회 | 테마: "${theme}" | 키워드: [${keywords.join(", ")}] | 결과: ${snapshot.size}개`,
    );
    const results: PlanItem[] = [];
