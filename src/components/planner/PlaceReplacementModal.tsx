...
export default function PlaceReplacementModal({ isOpen, onClose, onReplace, originalItem }: PlaceReplacementModalProps) {
  ...
  const handleApply = () => {
    ...
    const place = MOCK_RECOMMENDATIONS.find(p => p.id === selectedPlaceId);
    if (!place) return;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in p-4 pb-0 md:p-4">
      <div className="bg-white w-full md:w-[500px] md:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-slide-up">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-full cursor-pointer text-xs font-bold whitespace-nowrap transition-all
              ...
              {filteredPlaces.map((place) => {
                ...
                return (
                  <div 
                    ...
                    className={`p-4 rounded-xl border shadow-sm transition-all flex justify-between items-center cursor-pointer relative overflow-hidden group
                        ${isSelected
