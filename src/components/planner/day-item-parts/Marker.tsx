 ...
 * @param hasRainRisk 강수 위험이 있는지 여부
 */
export function Marker({ index, selected, hasRainRisk }: MarkerProps) {
    return (
        <div
            className={cn(
                "w-7 h-7 md:w-9 md:h-9 rounded-full border-[3px] font-bold text-[10px] md:text-xs flex items-center justify-center",
                "z-10 shrink-0 shadow-sm mt-1 transition-all duration-300 relative",
                // 선택되거나 비 위험이 있을 때 강조 스타일
                (selected || hasRainRisk)
